...
class DecodeDataset(CnnDmDataset):
    ...
    def __getitem__(self, i):
        ...
        art_sents = js_data['edu']
        #label = js_data['sentence']
        label = [0 for _ in range(len(art_sents))]
        return art_sents, label
...
class Abstractor(object):
    ...
    def _prepro(self, raw_article_sents, masks):
        ...
        masks = pad_batch_tensorize(masks, 0, cuda=False
                                    ).to(self._device)
        ...
        dec_args = (article, art_lens, extend_art, extend_vsize, masks,
                    START, END, UNK, self._max_len)
        return dec_args, ext_id2word

    def __call__(self, raw_article_sents, masks):
        self._net.eval()
        dec_args, id2word = self._prepro(raw_article_sents, masks)
...
class BeamAbstractor(Abstractor):
    def __call__(self, raw_article_sents, masks, beam_size=5, diverse=1.0):
        self._net.eval()
        dec_args, id2word = self._prepro(raw_article_sents, masks)
...
class RLExtractor(object):
    ...
    def __call__(self, raw_article_sents, sent_labels):
        self._net.eval()
        indices = self._net(raw_article_sents, sent_labels)
        return indices
