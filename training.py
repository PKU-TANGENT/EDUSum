...
class BasicPipeline(object):
    def __init__(self, name, net,
                 ...
                 val_fn, criterion, optim, grad_fn=None, model_path=None):
        self.name = name
        self._net = net
...
class BasicTrainer(object):
    ...
    def log(self, log_dict):
        ...
        print('train step: {}, {}: {:.4f}  '.format(
            self._step,
            'loss' if 'loss' in log_dict else 'reward',
            self._running_loss), end='')
        if 'advantage' in log_dict.keys():
            print('now loss: {:.4f}  '.format(log_dict['reward']), end='')
            print('advantage : {:.4f}  '.format(log_dict['advantage']), end='')
    ...
    def checkpoint(self):
        self._pipeline.checkpoint(
            join(self._save_dir, 'ckpt'), self._step)
