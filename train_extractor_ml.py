...
from model.util import multi_target_sequence_loss, sequence_loss
...
class ExtractDataset(CnnDmDataset):
    ...
    def __getitem__(self, i):
        ...
        art_sents, extracts, sentence = js_data['edu'], js_data['extracted'], js_data['sentence']
        return art_sents, extracts, sentence
...
def configure_training(net_type, opt, lr, clip_grad, lr_decay, batch_size):
    ...
    if net_type == 'ff':
        ...
        ce = lambda logit, target: F.cross_entropy(
            logit, target, reduce=False)
...
def main(args):
    ...
    pipeline = BasicPipeline(meta['net'], net,
                             ...
                             criterion, optimizer, grad_fn=grad_fn, model_path=args.model)
