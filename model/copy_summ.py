...
class CopySumm(Seq2SeqSumm):
    ...
    def forward(self, article, art_lens, abstract, extend_art, extend_vsize, attent_mask):
        attention, init_dec_states = self.encode(article, art_lens)
        #for a, e in zip(attention, attent_mask):
        #    assert len(a) == len(e)
        #mask = torch.tensor(attent_mask).to(attention.device).unsqueeze(-2)
    ...
    def batch_decode(self, article, art_lens, extend_art, extend_vsize, attent_mask,
                     go, eos, unk, max_len):
    ...
    def batched_beamsearch(self, article, art_lens,
                           extend_art, extend_vsize, attent_mask,
