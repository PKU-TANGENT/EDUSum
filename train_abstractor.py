...
from model.util import sequence_loss, edu_to_sentence
...
class MatchDataset(CnnDmDataset):
    ...
    def __getitem__(self, i):
        ...
        art_sents, abs_sents, extracts = (js_data['edu'], js_data['abstract'], js_data['extracted'])
        matched_arts = []
        matched_mask = []
        for e in extracts:
            tmp = ""
            for i in e:
                tmp += art_sents[i] + ' . '
            #print(tmp)
            matched_arts.append(tmp)
            matched_mask.append([1] * len(tmp.split()))
        '''
        matched_arts = []
        matched_mask = []
        for e in extracts:
            ...
            mask = []
            if len(e) == 1:
                sent, mask = edu_to_sentence(art_sents, sent_label, e[0])
            else:
                if sent_label[e[0]] == sent_label[e[1]]:
                    sent, mask = edu_to_sentence(art_sents, sent_label, e, True)
                else:
                    for i in e:
                        s, m = edu_to_sentence(art_sents, sent_label, i)
                        sent.append(s)
                        mask.append(m)
                    sent = ' '.join(sent)
                    m = []
                    for x in mask:
                        m += x
                    mask = m
            matched_arts.append(sent)
            matched_mask.append(mask)
        '''
        return matched_arts, abs_sents[:len(extracts)], matched_mask
...
def main(args):
    ...
    pipeline = BasicPipeline(meta['net'], net,
                             ...
                             criterion, optimizer, grad_fn, model_path=args.model)
...
if __name__ == '__main__':
    ...
    # length limit
    parser.add_argument('--max_art', type=int, action='store', default=200,
