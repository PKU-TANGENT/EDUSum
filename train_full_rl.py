...
class RLDataset(CnnDmDataset):
    ...
    def __getitem__(self, i):
        ...
        sent_label = js_data['sentence']
        return art_sents, abs_sents, sent_label
...
def build_batchers(batch_size):
    def coll(batch):
        art_batch, abs_batch, sent_label = unzip(batch)
...
def train(args):
    ...
    #stop_reward_fn = lambda x, y : 0.3*compute_rouge_n(x, y, n=1)+ 1.6 * compute_rouge_n(x, y, n=2)
    ...
    pipeline = A2CPipeline(meta['net'], agent, abstractor,
                           ...
                           stop_reward_fn, args.stop, args.model)
